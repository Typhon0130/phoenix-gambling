  ...
  export default {
    ...
    methods: {
      ...
      start() {
        ...
        window.axios.post('/admin/chat_bot/start').then(() => {
          this.$toast.success('Done');
        });
      }
    },
    created() {
      window.axios.post('/admin/chat_bot/status').then(({ data }) => this.status = data.status);
      window.axios.post('/admin/chat_bot/settings').then(({ data }) => this.data = data);
    },
    components: {
      WebIcon
    }

...
class ChatMessageWhisper extends WebSocketWhisper {
  ...
  public function process($data): array {
    ...
    if(Settings::get('quiz_active') === 'true') {
      ...
      if($sanitize($data->message) === $sanitize(Settings::get('quiz_answer'))) {
        Settings::set('quiz_active', false);
        $this->user->balance($this->user->clientCurrency())->add(floatval($this->user->clientCurrency()->option('quiz')), \App\Models\Transaction::builder()->message('Quiz')->get());
        event(new \App\Events\QuizAnswered($this->user, Settings::get('quiz_question'), Settings::get('quiz_answer')));
      }
    }

...
class Roulette extends QuickGame implements MultiplierCanBeLimited {
  ...
  function start($user, Data $data) {
    ...
    if ($totalBet < Currency::find($data->currency())->minBet() || ($user != null && $user->balance(Currency::find($data->currency()))->demo($data->demo())->get() < $totalBet))
      return new RejectedGameResult(-1, 'Invalid wager value (chips)');
    if($totalBet > Currency::find($data->currency())->convertEURToToken(20))
      return new RejectedGameResult(-1, 'Invalid wager value (chips)');
    $data->bet($totalBet);

...
class MatchWinnerThreeway extends PhoenixGamblingEsportMarketHandler {

  function isHandling(string $market, string $runner): bool {
    return $market === 'Match winner - threeway';
  }
  ...
  function isWinner(string $runner, SportGameSnapshot $snapshot): string {
    ...
    if($runner === 'draw') {
      ...
      return $this->lose();
    } else if($runner === $stats->game()->home) {
      if($stats->scores()->home() > $stats->scores()->away()) return $this->win();
      return $this->lose();
  ...
  function translation(string $market, string $runner): SportMarketTranslation {
    return (new SportMarketTranslation())->runner($runner)->market('sport.market.esport.matchWinnerThreeWay');
  }

  ...
  export default {
    ...
    methods: {
      ...
      restore(game) {
        ...
        window._.forEach(game.history, (e) => {
      ...
      extendedAutoBetHandle(take) {
        ...
        let picked = [];
        window._.forEach($('.game-mines').find('.autoBetPick'), (e) => picked.push(parseInt($(e).attr('data-mine-id'))));
      ...
      turn(mineId, callback = null) {
        if(window.$minesBtnTimeout) return;
        window.$minesBtnTimeout = true;

        this.sendTurn({id: mineId}, (response) => {
          if (response.type === 'fail') {
            window.$minesBtnTimeout = false;
            return;
          }
          ...
          if (Array.isArray(mineId)) window._.forEach(mineId, (id) => $(`[data-mine-id="${id}"]`).addClass('selected'));
          ...
          {name: 'label', data: {label: this.$i18n.t('general.mines')}},
          {
            name: 'buttons', data: {
              buttons: [
                ...
                {
                  label: this.$i18n.t('general.edit'),
                  callback: (v) => this.mines = v,
                  type: 'input',
                  input: {min: 2, max: 24}
                }]
            }

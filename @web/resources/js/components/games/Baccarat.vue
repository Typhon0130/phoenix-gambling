<template>
    <div>
        ...
        <div class="baccarat-field">
            <div class="header">
                {{ $t('general.wager') }}: <span id="bet">{{ rawBitcoin(currency, 0) }}</span> <web-icon :icon="currencies[currency].icon" :style="{ color: currencies[currency].style }"></web-icon>
                <div class="right">
    ...
    methods: {
        ...
        addPlayer(data) {
            _.forEach(data.data.bet, (value, key) => {
                ...
                $(`[data-container="${key}"] .os-content`).append(`<div class="user">

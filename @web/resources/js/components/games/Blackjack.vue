...
<script>
  ...
  class Player {
    ...
    hit(dbl) {
      if(window.$bjBtnTimeout) return;
      window.$bjBtnTimeout = true;

      setTimeout(() => {
        window.$bjBtnTimeout = false;
      }, 500);
    ...
    double() {
      ...
      setTimeout(() => window.$bjBtnTimeout = false, 500);
    ...
    getValue() {
      switch (this.getRank()) {
        ...
        case 'J':
          return 10;
        default:
          return parseInt(this.getRank(), 0);
      }
    }
  ...
  export default {
    ...
    methods: {
      ...
      renderCard(ele, sender, item, isHiddenByServer, secretRevealOptions) {
        ...
        let rank = card.getRank(), suit = card.getSuit(), posx = 0, posy = 20, speed = 200, cards = ele + ' .card-' + i,
          type = card.getType();
        if (i > 0) posx -= 50 * i;
        ...
        return [
          ...
          {name: 'wager-classic'},
          {
            name: 'buttons', data: {
              buttons: [
                {
                  label: this.$i18n.t('general.stand'),
                  class: 'stand',
                  preventMark: true,
                  isAction: true,
                  callback: () => this.stand()
          ...
          },
          {
            name: 'buttons', data: {
              buttons: [
                {
                  label: this.$i18n.t('general.double'),
                  class: 'double',
                  preventMark: true,
                  isAction: true,
                ...
                {
                  label: this.$i18n.t('general.split'),
                  class: 'splitB',
                  preventMark: true,
                  isAction: true,
                  callback: () => this.prfSplit()
                }
              ]
            }
          },

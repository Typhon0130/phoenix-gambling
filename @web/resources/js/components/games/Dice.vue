  ...
  export default {
    ...
    mounted() {
      $('.game-dice').find('.dice-slider').slider({
        ...
        slide: (event, ui) => {
          ...
          if (this.currentTarget === 'lower' && ui.value >= 97) return false;
          if (this.currentTarget === 'higher' && ui.value <= 4) return false;
      ...
      $('.game-dice').find('.dice-append').on('click', () => {
        ...
        let v = 100 - $('.game-dice').find('.dice-slider').slider('value');
        if (this.currentTarget === 'higher' && v <= 6) v = 6;
        if (this.currentTarget === 'lower' && v >= 94) v = 94;
    ...
    methods: {
      handleNumInput(val) {
        ...
        if (isNaN(value) || (this.currentTarget === 'lower' && value > 95) || (this.currentTarget === 'higher' && value < 6) || value < 6 || value > 95) {
      ...
      update() {
        ...
        Bus.$emit('profit:update', this.getProfit(this.currentTarget === 'lower' ? 0 : value, this.currentTarget === 'higher' ? 100 : value));
        $('.dice-chance').val((this.currentTarget === 'higher' ? 100 - value : value) + '%');

...
<script>
    ...
    export default {
        methods: {
            ...
            restore(game) {
                $('.coinflip-color').removeClass('disabled');
                window._.forEach(game.history, (color) => Bus.$emit('coinflip:history:addEntry', { html: `<div class="coin ${color === 'yellow' ? 'front' : 'back'}"><div class="front"></div><div class="back"></div></div>` }));
            },

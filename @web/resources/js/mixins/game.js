...
Vue.mixin({
  mounted() {
    ...
    const validateMultiplayerAction = (game_id, event, data, log = true) => {
      if (this.gameInstance.game && this.gameInstance.game.id === game_id) {
        if (log) console.log(`mWS ${event}`, data);
  ...
  created() {
    const setWebMode = (path) => {
      this.isCasino = !path.includes('/sport') && !path.includes('/esport');
      this.sportType = this.isCasino ? null : (path.includes('/sport') ? 'SPORTS' : (path.includes('/esport') ? 'ESPORTS' : null));
      this.sportLink = this.sportType === null ? null : (this.sportType === 'SPORTS' ? 'sport' : (this.sportType === 'ESPORTS' ? 'esport' : null));
    }

    setWebMode(window.location.href);

    Bus.$on('route:pathChange', ({ to }) => {
      setWebMode(to);
    });
  },
  methods: {
    ...
    rawBitcoin(to, value) {
      if (this.currencies[to].decimals === 2) return (value ? value : 0).toFixed(2);
      return bitcoin(value, 'btc').to(this.unit).value().toFixed(this.unit === 'satoshi' ? 0 : 8);
    },
    fiatToToken(price, usd) {
      return usd / price;
    },
    tokenToFiat(price, token) {
      return token * price;
    },
    ...
    playSound(src, timeout = null) {
      if (timeout != null) {
        let cancel = false;
        window._.forEach(this.soundTimeouts, function (t) {
    ...
    resultPopup(game) {
      ...
      const resultPopup = $(`<div class="resultPopup resultPopup-${status}" ${this.demo ? `style="padding-top: 25px;"` : ''}>
                    ...
                    <div class="multiplier">${status === 'lose' && game.multiplier >= 1 ? '0.00' : game.multiplier.toFixed(2)}x</div>
                    <div class="divider"></div>
                    <div class="profit">${game.profit.toFixed(game.currency.startsWith('local_') ? 2 : 8)} <web-icon :icon="fad fa-gem"></div>
                </div>
            `);

(function($) {
  ...
  SuperWheel = (function() {
    ...
    function SuperWheel(element, settings) {
      ...
      dataSettings = window.$(element).data('superWheel') || {};
      self.o = window.$.extend({}, self.defaults, settings, dataSettings);

      if(typeof self.o.text   !== 'object') self.o.text   = self.defaults.text;
      else self.o.text = window.$.extend({}, self.defaults.text, self.o.text);

      if(typeof self.o.slice  !== 'object') self.o.slice  = self.defaults.slice;
      else self.o.slice = window.$.extend({}, self.defaults.slice, self.o.slice);

      if(typeof self.o.slice.selected !== 'object') self.o.slice.selected = self.defaults.slice.selected;
      else self.o.slice.selected = window.$.extend({}, self.defaults.slice.selected, self.o.slice.selected);

      if(typeof self.o.line  !== 'object') self.o.line  = self.defaults.line;
      else self.o.line = window.$.extend({}, self.defaults.line, self.o.line);

      if(typeof self.o.outer  !== 'object') self.o.outer  = self.defaults.outer;
      else self.o.outer = window.$.extend({}, self.defaults.outer, self.o.outer);

      if(typeof self.o.inner  !== 'object') self.o.inner  = self.defaults.inner;
      else self.o.inner = window.$.extend({}, self.defaults.inner, self.o.inner);

      if(typeof self.o.center !== 'object') self.o.center = self.defaults.center;
      else self.o.center = window.$.extend({}, self.defaults.center, self.o.center);
      ...
      else self.o.marker = window.$.extend({}, self.defaults.marker, self.o.marker);

      window.$.each(self.o.slices, function(i, slice) {
      ...
      self.initials = {
        ...
        width : 400,
        cache: window.$(element).data('superWheelData') ? window.$(element).data('superWheelData').cache : []
      };
      window.$.extend(self, self.initials);
      self.half = 200 / 2;
      window.$.extend(window.$.easing, {superWheel: function(x, t, b, c, d) {return -c * ((t = t / d - 1) * t * t * t - 1) + b}});
      window.$.extend(window.$.easing, {easeOutQuad: function(x, t, b, c, d) {return -c * (t /= d) * (t - 2) + b;}});
      self.instanceUid = window.$(element).data('superWheelData') ? window.$(element).data('superWheelData').instanceUid : instanceUid++;
      window.$(element).data('superWheelData',self);
      self.init();
    }
    return SuperWheel;
  }());
  ...
  SuperWheel.prototype.init = function() {
    ...
    var arcSize = 360 / self.totalSlices(),
      pStart = 0,
      pEnd = 0,
      wrapper = window.$('<div/>').addClass('sWheel-wrapper').appendTo(self.$element),
      inner = window.$('<div/>').addClass('sWheel-inner').appendTo(wrapper),
      spinner = window.$('<div/>').addClass('sWheel').prependTo(inner),
      Layerbg = window.$('<div/>').addClass('sWheel-bg-layer').appendTo(spinner),
      Layersvg = window.$(self.SVG('svg', {
        'version': '1.1',
      ...
      mask = window.$('<defs><clipPath id="cut-off-line"></circle></clipPath></defs>'),
    ...
    if ( ( self.o.text.orientation === 'v' || self.o.text.orientation === 'vertical') ||
      ...
      var textsGroup = window.$('<g class="sWheel-txt"/>'),
        LayerDefs = window.$('<defs/>');
    }
    var Layercenter = window.$('<div/>');
    ...
    if (typeof self.o.center.image.url === 'string' && window.$.trim(self.o.center.image.url) !== '') {
      var centerImage = window.$('<img />');
    ...
    if (typeof self.o.center.html.template === 'string' && window.$.trim(self.o.center.html.template) !== '') {
      var centerHtml = window.$('<div class="sw-center-html">' + self.o.center.html.template + '</div>');
    ...
    if (window.$.trim(self.o.type) !== 'color') {
      var Layermarker = window.$('<div/>').addClass('sWheel-marker').appendTo(wrapper);
    ...
    var slicesGroup = window.$('<g/>'),
      ...
      cy = self.half,
      cr = 0;
    window.$.each(self.slices, function(i, slice) {
      /*#######################################################
      /////////////////////////////////////////////////////////
      //////////////////   Slice Drawer   /////////////////////
      /////////////////////////////////////////////////////////
      ...
      if ( ( self.o.text.orientation === 'v' || self.o.text.orientation === 'vertical') ||
        ...
        var LayerText = window.$('<text stroke-width="0" data-color="' + slice.color + '" fill="' + slice.color + '" dy="' + (self.toNumber(self.o.text.offset)) + '%">' + '<textPath xlink:href="#sw-text-' + i + '" startOffset="50%" style="text-anchor: middle;">' + slice.text + '</textPath>' + '</text>');
      ...
      var LayerTitleInner = window.$('<div/>');
    ...
    window.$(outerLine).appendTo(Layersvg);
    ...
    window.$(innerLine).appendTo(Layersvg);
    ...
    self.FontScale();
    window.$(window).on('resize.' + self.instanceUid, function() {
      self.$element.height(self.$element.width());
      self.$element.find('.sWheel-wrapper').width(self.$element.width());
      self.$element.find('.sWheel-wrapper').height(self.$element.width());
      self.FontScale();
  ...
  SuperWheel.prototype.start = function(key,val) {
    ...
    if(typeof self.cache['onStart'] !== 'undefined'){
      window.$.each(self.cache['onStart'], function(i, callback) {
    ...
    if (parseInt(self.o.frame,10) !== 0) {
      var oldinterval = window.$.fx.interval;
      window.$.fx.interval = parseInt(self.o.frame,10);
    }
    self.spinner = window.$({
      deg: self.now
    }).animate({
      ...
      easing: window.$.trim(self.o.easing),
      step: function(now, fx) {
        if (parseInt(self.o.frame,10) !== 0) window.$.fx.interval = parseInt(self.o.frame,10);
        self.now = now % 360;
        if (window.$.trim(self.o.type) !== 'color') {
        ...
        if(typeof self.cache['onProgress'] !== 'undefined'){
          window.$.each(self.cache['onProgress'], function(i, callback) {
            if(typeof callback === 'function')
        ...
        if (self.lastStep !== self.currentStep) {
          ...
          if ((self.o.marker.animate === true || self.o.marker.animate === 1 || self.o.marker.animate === 'true' ) && window.$.inArray(window.$.trim(self.o.easing), ['easeInElastic', 'easeInBack', 'easeInBounce', 'easeOutElastic', 'easeOutBack', 'easeOutBounce', 'easeInOutElastic', 'easeInOutBack', 'easeInOutBounce']) === -1) {
          ...
          if (window.$.trim(self.o.type) === 'color') {
            self.$element.find('svg>g.sw-slicesGroup>path').each(function(i) {
              window.$(this).attr('class','').attr('fill', window.$(this).attr('data-fill'));
            });
            self.$element.find('.sWheel-txt>.sWheel-title').each(function(i) {
              window.$(this).attr('class','sWheel-title');
              if (self.o.text.orientation === 'v' || self.o.text.orientation === 'vertical') window.$(this).css('color', window.$(this).attr('data-color'));
              else window.$(this).attr('fill', window.$(this).attr('data-color'));
            });
          ...
          if(typeof self.cache['onStep'] !== 'undefined'){
            window.$.each(self.cache['onStep'], function(i, callback) {
              if(typeof callback === 'function')
                callback.call(self.$element, self.currentSliceData.results, self.slicePercent, self.circlePercent);
            });
          }
        }
        if (parseInt(self.o.frame,10) !== 0) window.$.fx.interval = oldinterval;
      },
      ...
      complete: function(animation, progress, remainingMs) {
        ...
        if(typeof self.cache['onComplete'] !== 'undefined'){
          window.$.each(self.cache['onComplete'], function(i, callback) {
            if(typeof callback === 'function')
              callback.call(self.$element, self.slice.results, self.spinCount, self.now);
          });
        }
  ...
  SuperWheel.prototype.findWinner = function(value,type) {
    ...
    var filter = [];
    window.$.each(self.slices, function(i, slice) {
      if(typeof slice[self.o.selector] === 'object' || Array.isArray(slice[self.o.selector]) || typeof slice[self.o.selector] === 'undefined')
        return undefined;
  ...
  window.$.fn.wheel = function() {
    ...
    for ( i = 0; i < l; i++  ) {
      ...
      } else if (window.$.inArray(window.$.trim(opt), allowed) !== -1) {
        if (window.$.trim(opt) === 'option') {
  ...
  window.$bus.$emit('wheel:initCompleted');
})();
